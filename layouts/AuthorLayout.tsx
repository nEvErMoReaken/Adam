...
const PROJECTS = [
  {
    nameZh: 'MSS / IoT网关',
    nameEn: 'MSS / IoT Gateway',
    roleZh: '核心开发+架构',
    roleEn: 'Core Dev + Arch',
    running: true,
    since: '2023-07',
  },
  {
    nameZh: 'BYD充电站智能化',
    nameEn: 'BYD Smart Station',
    roleZh: 'PM+算法工程化',
    roleEn: 'PM + Algo Eng',
    running: true,
    since: '2025-07',
...
function Divider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-1 py-1 font-mono text-[10px] text-[var(--c-overlay0)] select-none">
      <span>+-{'-'.repeat(14)}-+</span>
      <span className="shrink-0 tracking-widest uppercase">{label}</span>
      <span>+-{'-'.repeat(14)}-+</span>
    </div>
  )
}
  ...
  return (
    <PaneLayout cols="grid-cols-1">
      <Pane title={t.smiTitle} index={0}>
        <div className="space-y-2 overflow-x-auto p-4 font-mono text-xs">
          {/* 顶部框 */}
          ...
          <div className="space-y-0.5">
            {PROJECTS.map((p) => (
              <p key={p.nameEn} className="flex gap-1">
                ...
                <span className="w-24 truncate text-[var(--c-blue)]">
                  {lang === 'zh' ? p.nameZh : p.nameEn}
                </span>
                <span className="w-20 truncate text-[var(--c-text)]">
                  {lang === 'zh' ? p.roleZh : p.roleEn}
                </span>
                <span
                  style={{ color: p.running ? 'var(--c-green)' : 'var(--c-subtext0)' }}
          ...
          <div className="space-y-0.5">
            {Object.entries(SKILLS).map(([category, items]) => (
              <p key={category} className="flex flex-wrap gap-x-2 gap-y-0.5">
                ...
                <span className="text-[var(--c-overlay0)]">:</span>
                {items.map((s) => (
                  <span key={s} className="text-[var(--c-text)]">
                    {s}
                  </span>
                ))}
              </p>
          ...
          <div className="pt-1 text-[var(--c-overlay0)] select-none">
            <p>+{'─'.repeat(54)}+</p>
          </div>
        </div>
      </Pane>
