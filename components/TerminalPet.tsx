  ...
  return (
    ...
    >
      ...
      {STAT_NAMES.map((s) => (
        <div
          key={s}
          style={{ display: 'flex', justifyContent: 'space-between', gap: 6, marginBottom: 2 }}
        >
          <span
            style={{ color: STAT_COLORS[s], minWidth: isZh ? 36 : 72, fontSize: isZh ? 11 : 10 }}
          >
            {isZh ? STAT_ZH[s] : s}
          </span>
          <span style={{ color: STAT_COLORS[s] }}>
            <StatBar value={buddy.stats[s]} />
          </span>
        </div>
      ))}
      ...
      >
        <button
          ...
          onMouseDown={(e) => {
            e.stopPropagation()
            document.getElementById('hatch-trigger')?.click()
          }}
        >
          {isZh ? '重新孵化' : 'reroll'}
        </button>
                ...
                style={{
                  ...
                  animation:
                    hatchPhase === 'born'
                      ...
                      : buddy?.shiny
                        ? 'petshine 2s ease-in-out infinite'
                        : undefined,
                  pointerEvents: 'none',
                }}
              >
