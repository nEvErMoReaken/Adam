...
export default function GoGateBufferViz() {
  ...
  useEffect(() => {
    rpRef.current = readPos
  }, [readPos])
  useEffect(() => {
    wpRef.current = writePos
  }, [writePos])
  ...
  return (
    <div
      className="my-6 overflow-hidden rounded-md border font-mono text-[11px]"
    ...
    >
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {[
            {
              label: isReading ? '⏸ 停止读取' : '▶ 开始读取',
              action: () => setIsReading((r) => !r),
              active: isReading,
              color: 'var(--c-blue)',
            },
            {
              label: isWriting ? '⏸ 停止写入' : '▶ 开始写入',
              action: () => setIsWriting((w) => !w),
              active: isWriting,
              color: 'var(--c-mauve)',
            },
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col items-center">
            <svg width="220" height="220" viewBox="0 0 220 220">
                    ...
                    fill={
                      isR
                        ...
                        : isW
                          ? 'var(--c-mauve)'
                          : filled
                            ? 'color-mix(in srgb, var(--c-blue) 50%, var(--c-surface0))'
                            : 'var(--c-surface1)'
                    }
              ...
              {/* Pointer spokes */}
              {[
                { pos: rA, color: 'var(--c-blue)', show: avail > 0 },
                { pos: wA, color: 'var(--c-mauve)', show: avail < SIZE },
              ].map(({ pos, color, show }) => {
                if (!show) return null
              ...
              <text x={CX} y={CY + 5} textAnchor="middle" fontSize="8" fill="var(--c-overlay0)">
                avail
              </text>
              <text x={CX} y={CY + 17} textAnchor="middle" fontSize="8" fill="var(--c-overlay0)">
                {free} free
              </text>
            </svg>
            ...
            <div className="flex gap-3 text-[9px]" style={{ color: 'var(--c-overlay0)' }}>
              <span>
                <span style={{ color: 'var(--c-blue)' }}>●</span> readPos[{rA}]
              </span>
              <span>
                <span style={{ color: 'var(--c-mauve)' }}>●</span> writePos[{wA}]
              </span>
            </div>
          </div>
          ...
          {/* Log */}
          <div
            className="min-h-[180px] rounded px-3 py-2 leading-relaxed"
        ...
        {/* Live bit-op */}
        <div
          className="rounded px-4 py-2.5 leading-loose"
          style={{ background: 'var(--c-surface0)' }}
        >
          <div
            className="mb-0.5 text-[9px] tracking-widest uppercase"
            style={{ color: 'var(--c-overlay0)' }}
          >
            位运算取模（实时）
          </div>
          ...
          <div style={{ color: 'var(--c-subtext0)' }}>
            ...
            <span className="font-bold" style={{ color: 'var(--c-mauve)' }}>
              {wA}
            </span>
            {writePos >= SIZE && (
